...
export default function TwitterCall(props) {
    ...
    useEffect(() => {
        ...
        const getData = async () => {
            ...
            // content
            // const response = await fetch(SameOriginWorkaround + `https://api.twitter.com/2/tweets/search/recent?max_results=50&query=${hashTag}${queryParams}${paginationParam}`, myHeaders)
            
            // author of tweet
            // const response = await fetch(SameOriginWorkaround + `https://api.twitter.com/2/tweets/search/recent?max_results=50&query=from%3A${userName}${queryParams}${paginationParam}`, myHeaders)
            // const response = await fetch(SameOriginWorkaround + `https://api.twitter.com/2/tweets/search/recent?max_results=50&query=from%3A${authorId}${queryParams}${paginationParam}`, myHeaders)
    ...
    tweetRenderData = tweetRenderData.sort((c1, c2) => (c1.text < c2.text) ? -1 : (c1.text > c2.text) ? 1 : 0);
                ...
                if (existingImages.indexOf(tweet.imageUrl) < 0) {
                    ...
                    return (
                        <div key={index} >
                            ...
                            <img src={tweet.imageUrl} style={{"width" : "300px", "height" : "auto"}}></img>
